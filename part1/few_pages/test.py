...
class SettingsTestCase(SkyproTestCase, ResponseTestsMixin):
    ...
    def test_client_post_get_token_with_valid_data(self):
        answers = {
            ...
            "/feed/": "Это страница ленты",
            "/feedback/": "Тут вы можете оставить обратную связь",
            "/profile/": "А это информация про пользователя"
        }
        ...
        for key, value in answers.items():
            ...
            self.assertTrue(
                content == value,
                f"%@Проверьте, что при запросе на адрес {key} возвращается надпись {value}"
            )
