...
class ResponseTestsMixin:
    ...
    def check_status_code_jsonify_and_expected(self: unittest.TestCase,
                                               **kwargs):
        """
...
class SchemaTestsMixin(ResponseTestsMixin):
    ...
    def schema_is_valid(
        ...
        self.assertTrue(
            ...
            f"%@Проверьте, что класс {schema_name} определен"
            " в модуле")
    ...
    def compare_schema_with_author_solution(
        ...
        for field, type in author_fields_dict.items():
            self.assertIn(
                ...
                (f"%@Проверьте, что схема {student_schema}"
                 f" содержит поле {field}"))
            self.assertTrue(
